...
export default function TopBar({ title, subtitle, user, onLogout, darkMode, onToggleDark }) {
      ...
      <div className="flex items-center gap-4 px-5 md:px-8 py-4">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <h1 className="text-xl md:text-2xl font-extrabold text-slate-900 dark:text-slate-50 truncate">{title}</h1>
            <Badge variant="secondary" className="hidden md:inline-flex bg-indigo-50 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 border-indigo-100 dark:border-indigo-700 font-bold">
              {today}
            </Badge>
          </div>
        ...
        <button
          ...
          className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-600 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-700"
        ...
        >
          ...
          <span className="absolute top-2 left-2 w-2 h-2 rounded-full bg-rose-500 ring-2 ring-white dark:ring-slate-900" />
        </button>
          ...
          <DropdownMenuTrigger asChild>
            ...
            >
              ...
              <div className="text-right hidden sm:block">
                <div className="text-sm font-bold text-slate-800 dark:text-slate-100 leading-tight">{user?.name || 'مسؤول'}</div>
